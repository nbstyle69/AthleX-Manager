...
export async function getMyBox(
  ...
  if (error) {
    // `null` voudrait dire « cet utilisateur n'administre aucune box ». Une
    // panne de la RPC n'est pas cette phrase-là : on la laisse remonter à
    // l'écran plutôt que de la déguiser en absence de box.
    throw new Error(`get_my_admin_boxes a échoué : ${error.message}`);
  }

  const boxes = (data ?? []) as { id: string; my_role: 'owner' | 'coach' }[];
