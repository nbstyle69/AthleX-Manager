...
const SITES: Site[] = [
  ...
  { id: 'C2 décider les matchs', file: BRACKET, opens: 'onClick={() => autoResolveRound(lastRound)}', direct: 'decideRound(lastRound', run: /function autoResolveRound\(round: number\) \{\s*const wod = wodForRound\(round\);\s*ask\(\{[\s\S]{0,900}run: \(\) => decideRound\(round, wod\?\.id \?\? null\),\s*\}\);\s*\}/ },
