...
const SITES: Site[] = [
  ...
  { id: 'C2 décider les matchs', file: BRACKET, opens: 'onClick={() => autoResolveRound(lastRound)}', direct: 'applyAutoResolve(lastRound', run: /const skipped = pending\.length - decisions\.length;\s*ask\(\{[\s\S]{0,700}run: \(\) => applyAutoResolve\(round, decisions\),\s*\}\);\s*\}/ },
