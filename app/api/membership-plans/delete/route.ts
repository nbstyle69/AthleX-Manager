 ...
 *   en impayé rattachés (la boîte de confirmation choisit ses textes).
 ...
 *   (journal + e-mail « fin de période », acteur = le gérant) ; la formule
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: subsRaw } = await supabase
      .from('box_members')
      .select('id, box_id, member_id, stripe_subscription_id, subscription_status, subscription_cancel_at_period_end, subscription_current_period_end')
    ...
    if (action === 'check') {
      return NextResponse.json({ ok: true, active_subscriptions: subs.length });
    }
    ...
    if (action === 'delete') {
      if (subs.length > 0) {
        return NextResponse.json(
          {
            error: `${subs.length} abonnement(s) Stripe sont encore actifs sur cette formule : elle ne peut pas être supprimée.`,
    ...
    for (const s of subs) {
      ...
      if (s.subscription_cancel_at_period_end) continue;
      const profile = profileById.get(s.member_id) ?? null;
      try {
        const { emailed } = await stopBoxMember(supabase, {
          member: s, box, profile, planName: plan.name, mode: 'period_end', actorId: user.id,
        });
    ...
    if (failed.length > 0) {
      return NextResponse.json(
        {
          error: `Stripe a refusé l’arrêt de ${failed.length} abonnement(s) : ${failed.join(', ')}. La formule n’a pas été supprimée ; les autres abonnements sont bien programmés pour s’arrêter à la fin de leur période.`,
          failed,
    ...
    return NextResponse.json({
      ...
      ...(notEmailed > 0 ? { warning: `${notEmailed} membre(s) n’ont pas reçu l’e-mail : préviens-les directement.` } : {}),
    });
  } catch (err: any) {
